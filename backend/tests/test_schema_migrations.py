...
def test_unversioned_write_safety_create_all_requires_manual_fingerprint():
    columns = _legacy_columns(memory_enabled=True)
    ...
    with pytest.raises(LegacySchemaError, match="column names alone"):
        decide_schema_upgrade(
            current_revisions=(),
            existing_tables=columns,
            columns_by_table=columns,
...
@pytest.mark.parametrize("revision", ["001", "002", "003", "004", "005"])
...
def test_alembic_revision_chain_has_single_005_head():
    scripts = ScriptDirectory.from_config(make_alembic_config())

    assert scripts.get_current_head() == "005"
